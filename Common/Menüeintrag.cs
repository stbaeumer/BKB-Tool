...
zieldatei.Add("Der Unterricht endet nach der 5. Stunde um 12:00 Uhr.");
   ...
   string delimiter, char quote, Encoding encoding, bool shouldAllQuote, List<string> importhinweise = null)
 {
  ...
  // Für jeden gleichnamigen Kalender, der in Downloads liegt und nicht älter ist als configuratuin["MaxAlter"]
   ...
   var kalenderRec = Quelldateien.GetMatchingList(configuration, "termine.csv", IStudents, Klassen);
 ...
 private List<string> AktuelleCsv(List<string> fileNames, IConfiguration configuration)
 {        
    List<string> validFiles = new List<string>();

        int maxDateiAlterInTagen = Convert.ToInt32(configuration["MaxAlter"] ?? "3");
        // Stichtag: Genau 3 Tage vor dem aktuellen Zeitpunkt
        DateTime cutoffDate = DateTime.Now.AddDays(-maxDateiAlterInTagen);        

        foreach (string name in fileNames)
        {            
            // Pfad mit Dateiendung zusammensetzen
            string filePath = Path.Combine(configuration["PfadDownloads"], $"{name}.csv");

            // 1. Prüfen, ob die Datei existiert
            if (File.Exists(filePath))
            {
                // 2. Prüfen, ob die Datei nicht älter als 3 Tage ist
...
#pragma warning restore CS8604 // Possible null reference argument.
     ...
     string delimiter, char quote, Encoding encoding, bool shouldAllQuote, List<string> importhinweise = null)
 {
  ...
  try
  {
   foreach (var anrechnung in Anrechnungen.Where(a => a.Wert > 0).OrderBy(a => a.LehrerKuerzel).ThenBy(a => a.Grund))
   {
