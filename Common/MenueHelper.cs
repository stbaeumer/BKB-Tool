  ...
  try
  {
   ...
   return new Menue(
    ...
    [
     ...
     new Menüeintrag(
      "Gruppen & Organigramm: (Mo) Gruppen & Organigramm aus Untisanrechnungen und Unterrichten als SQLite-Dateien für Wiki-Import erstellen",
     ...
     new Menüeintrag(
      $"Stammdatenabgleiche: (Mo) Stammdaten zwischen SchILD und Untis abgleichen",
      new Dateien(),
      students,
      klassen,
      [
     ...
     new Menüeintrag(
      $"Zeugnis 1 - Unterrichte: (14 Tage vor den Zeugnissen) Unterrichte/Kurse schülergenau von Webuntis nach SchILD übertragen.",
     ...
     new Menüeintrag(
      $"Zeugnis 2 - Belegung: (13 Tage vor den Zeugnissen) In den Klassen des Beruflichen Gymnasiums die Klausurbelegung aus Wiki in die SchiLD-Leistungsdaten übernehmen",
      quelldateien.Notwendige(configuration, ["schuelerleistungsdaten,dat"]),
      students,
      klassen,
      [
       $"Die Kursarten ([{Global.GetColor(Global.ColorActionInMenüs)}]LK1, LK2, GKS, GKM, AB3, AB4[/]) und die Abiturfächer ([{Global.GetColor(Global.ColorActionInMenüs)}]1, 2, 3, 4[/]) werden in Wiki ausgelesen und in den SchuelerLeistungsdaten ergänzt. Die Klausurbelegung sollten auf der Wiki-Seite bereits nach dem vorangegangenen Halbjahresbeginn erfolgt sein. Idealerweise werden die SuS unmittelbar vor den Zeugnissen nochmals zur Kontrolle der Belegung in Wiki aufgefordert.",
       $"[{Global.GetColor(Global.ColorActionInMenüs)}]Hinweise:[/]",
       $"[{Global.GetColor(Global.ColorActionInMenüs)}]#1[/] Die Verarbeitung muss klassenweise erfolgen.",
       $"[{Global.GetColor(Global.ColorActionInMenüs)}]#2[/] Es werden nur Schuelerleistungsdaten in Zeilen des interessierenden Abschnitts ergänzt.",
       $"[{Global.GetColor(Global.ColorActionInMenüs)}]#3[/] Änderungen / Unstimmigkeiten zwischen Wiki und SchILD werden angezeigt."
      ],
      m =>
      {
        configuration = Global.Konfig("Abschnitt", Global.Modus.Update, configuration);
     ...
     new Menüeintrag(
      $"Zeugnis 3 - Fehlzeiten: ( 3 Tage vor den Zeugnissen) Die Fehlzeiten werden in bestehende Lernabschnittsdaten eingefügt",
      new Dateien(),
      students,
      klassen,
      [
     ...
     new Menüeintrag(
      $"Zeugnis 4 - Anmahnen: ( 2 Tage vor den Zeugnissen) LuL wegen fehlender Noten in Webuntis anmahnen",
      new Dateien(),
      students,
      klassen,
      [
     ...
     new Menüeintrag(
      $"Zeugnis 5 - Noten: ( 1 Tag  vor den Zeugnissen) Noten aus Webuntis nach SchILD importieren",
      new Dateien(),
      students,
      klassen,
      [
     ...
     new Menüeintrag(
      "Klassen anlegen: (vor den Sommerferien) Neue Klassen von Untis nach SchILD übergeben und Eigenschaften anpassen",
     ...
     new Menüeintrag(
      $"Altersermäßigung: (vor den Sommerferien) Altersermäßigung berechnen für {int.Parse(Global.AktSj[0])}/{int.Parse(Global.AktSj[0]) + 1} und {int.Parse(Global.AktSj[0]) + 1}/{int.Parse(Global.AktSj[0]) + 2}",
